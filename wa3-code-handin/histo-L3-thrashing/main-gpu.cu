...
template <int B>
void multiStepHisto(uint32_t *d_inp_inds, float *d_inp_vals, float *d_hist,
                    const uint32_t N, const uint32_t H, const uint32_t LLC) {
...
void runAll(const uint32_t N, const uint32_t H, const uint32_t LLC) {
  ...
  cudaMemcpy(d_inp_inds, inp_inds, N * sizeof(uint32_t),
...
int main(int argc, char *argv[]) {
  ...
  printf("Running GPU-Parallel Versions (Cuda) demonstrating LLC threshing on "
         "histogram example. N: %d, H: %d, LLC: %d\n",
         N, H, LLC);
