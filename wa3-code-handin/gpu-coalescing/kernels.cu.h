...
// widthA = heightB
template <class T>
__global__ void naiveTransposeKer(T *A, T *B, int heightA, int widthA) {
...
__global__ void coalsTransposeKer(ElTp *A, ElTp *B, int heightA, int widthA) {
 ...
 */
template <class ElTp>
__global__ void naiveKernel(ElTp *A, ElTp *B, uint32_t num_rows,
                            uint32_t num_cols) {
 ...
 **************************************************************/
template <class ElTp>
__global__ void transKernel(ElTp *A_tr, ElTp *B_tr, uint32_t num_rows,
                            uint32_t num_cols) {
  uint32_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= num_rows)
    return;
...
glb2shmem(uint32_t num_cols, uint32_t block_offs, uint32_t jj, uint32_t j,
          uint32_t chunk_lane, uint32_t chunk_id // input
          ,
          uint32_t *glb_ind, uint32_t *loc_ind // the result
) {
  *loc_ind = j * blockDim.x + threadIdx.x;
  *glb_ind = block_offs + j * (blockDim.x / CHUNK) * num_cols +
             chunk_id * num_cols + jj + chunk_lane;
}

template <class ElTp, int CHUNK>
__global__ void optimKernel(ElTp *A, ElTp *B, uint32_t num_rows,
                            uint32_t num_cols) { // unsigned int N) {
  ...
  for (int jj = 0; jj < num_cols; jj += CHUNK) {
    ...
    for (int j = 0; j < CHUNK; j++) {
      ...
      glb2shmem<CHUNK>(num_cols, block_offs, jj, j, chunk_lane, chunk_id,
                       &glb_ind, &loc_ind);
